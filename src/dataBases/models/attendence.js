...
//  return result.rows; 
// };
const getAttendenceByDateAndClassAndSection = async (class_name, section, date) => {
  const query = `SELECT attendence.id ,attendence.student_id,student_name, attendence_status, date
  FROM attendence 
  JOIN students ON attendence.student_id = students.id
  WHERE students.class_name = $1 
    AND students.section = $2
    AND attendence.date = $3`;
  

 console.log('class_name',class_name);
 console.log('values',[class_name, section,date]);
 const result = await db.query(query, [class_name, section, date]);
...
module.exports = {createAttendence, updateAttendenceDetails, getAttendenceByDateAndClassAndSection};